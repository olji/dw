...
# Character set usage
assert '$BIN --create-list=src.tmp --use-list=invocation_c_set.lst --use-config=c_set.conf' \
    'grep "ajib" invocation_c_set.lst' \
    0 \
    'INVOCATION - Character set used properly' \
    'invocation_cset.log'
# Allocation error
assert '$BIN --create-list=src.tmp --use-list=invocation_alloc_err.lst --use-config=alloc_error.conf' \
